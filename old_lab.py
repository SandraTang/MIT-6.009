...
def cost(structure, path, goal):
    ...
    # heuristic
    cost += dist_btw(structure, path[len(path)-1], goal)
    return cost
    ...
    while agenda:
        ...
        for node in connections:
            if node not in visited:
                ...
                new_path[0] = cost(aux_structures, new_path, node2)
                # print(new_path)
    ...
    while agenda:
        ...
        for node in connections:
            if node not in visited:
                new_path = path.copy()
                new_path.append(node)
                new_path[0] = time(aux_structures, new_path, node2)
                # print(new_path)
                agenda.append(new_path)
        visited.add(path_last_node)
    return None
